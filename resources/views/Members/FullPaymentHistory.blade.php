...
<html lang="en">
  <head>
    ...
    <title>Full Payment History | GBLDC </title>
    ...
    <div class="min-h-screen flex relative">
      ...
      <main class="flex-1 p-4 lg:p-6 ml-0 lg:ml-0">
        ...
        <div class="mt-12 lg:mt-0 mb-6">
          <h1 class="text-xl lg:text-2xl xl:text-3xl font-bold">Full Payment History</h1>
          <p class="text-gray-600 text-sm lg:text-base">View all your payments including Loan Payments and Shared Capital contributions.</p>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 mb-6">
          @php
            ...
            foreach($allPayments as $payment) {
                if($payment->transaction_type == 'Loan Payment') {
                    $totalLoanPayments += $payment->payment_amount;
                } elseif($payment->transaction_type == 'Shared Capital') {
                    $totalSharedCapital += $payment->payment_amount;
                }
            }
          @endphp
          ...
          <div class="bg-white rounded-lg shadow-md p-4">
            ...
            <p class="text-2xl font-bold text-teal-950">{{ $totalPayments }}</p>
          </div>
          <div class="bg-white rounded-lg shadow-md p-4">
            <h3 class="text-sm font-medium text-gray-500">Total Loan Payments</h3>
            <p class="text-2xl font-bold text-green-600">PHP {{ number_format($totalLoanPayments, 2) }}</p>
          </div>
          <div class="bg-white rounded-lg shadow-md p-4">
            <h3 class="text-sm font-medium text-gray-500">Total Shared Capital</h3>
            <p class="text-2xl font-bold text-blue-600">PHP {{ number_format($totalSharedCapital, 2) }}</p>
          </div>
        </div>
        ...
        <section class="bg-white rounded-lg shadow-md p-4 lg:p-6">
          <div class="flex justify-between items-center mb-4">
            <h2 class="text-lg lg:text-xl font-semibold text-teal-950">All Payment Transactions</h2>
            <a href="{{ route('Loan.Dashboard') }}" class="text-green-600 text-sm font-medium hover:underline">
            ...
            <table class="w-full text-left border-collapse min-w-[600px]">
              <thead>
                <tr class="bg-gray-100 text-gray-600">
                  ...
                  <th class="p-3 text-sm font-medium">Remarks</th>
                </tr>
              </thead>
              <tbody>
                ...
                <tr class="border-t hover:bg-gray-50">
                  <td class="p-3 text-sm">
                    <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium {{ $payment->transaction_type == 'Loan Payment' ? 'bg-green-100 text-green-800' : ($payment->transaction_type == 'Shared Capital' ? 'bg-blue-100 text-blue-800' : 'bg-gray-100 text-gray-800') }}">
                      {{ $payment->transaction_type }}
                    </span>
                  </td>
                  <td class="p-3 text-sm font-semibold">PHP {{ number_format($payment->payment_amount, 2) }}</td>
                  <td class="p-3 text-sm">{{ $payment->payment_method }}</td>
                  <td class="p-3 text-sm">
                    <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium {{ strtolower($payment->payment_status) == 'completed' ? 'bg-green-100 text-green-800' : (strtolower($payment->payment_status) == 'pending' ? 'bg-yellow-100 text-yellow-800' : 'bg-red-100 text-red-800') }}">
                      {{ $payment->payment_status }}
                    </span>
                  </td>
                  <td class="p-3 text-sm">{{ $payment->transaction_date }}</td>
                  <td class="p-3 text-sm font-mono text-xs">{{ $payment->reference_number }}</td>
                  <td class="p-3 text-sm text-gray-600">{{ $payment->remarks ?? 'N/A' }}</td>
                </tr>
                @empty
                <tr>
          ...
          <div class="mt-4 text-center text-sm text-gray-500">
            Showing all {{ count($allPayments) }} payment records
          </div>
        </section>
      </main>
