...
<html lang="en">
  ...
  <body class="bg-gray-50 text-gray-800 font-sans antialiased min-h-screen">
    ...
    <header class="bg-white shadow-md fixed left-0 top-0 z-50 w-full">
      <div
        ...
        <div class="flex items-center space-x-4">
          ...
          <h1 class="text-lg md:text-xl font-semibold text-teal-900">GBLDC Member List</h1>
        </div>
        <div class="relative">
    ...
    <!-- Title -->
    <div class="mb-8">
      <h2 class="text-2xl font-bold text-green-800">Member List</h2>
      <p class="text-gray-700">View all members.</p>
      ...
      <div class="overflow-x-auto">
        <table id="loanTable" class="display responsive nowrap w-full text-sm border border-green-100 rounded-lg">
          ...
          <tbody>
            ...
            <tr>
              <td>{{$AllMember->member_id}}</td>
              <td>{{$AllMember->last_name}}, {{$AllMember->first_name}} {{$AllMember->middle_name}}</td>
              <td>{{$AllMember->contact_number}}</td>
              <td>{{$AllMember->email}}</td>
              <td>{{$AllMember->created_at}}</td>
              <td>
                <a href="{{route('View.Member.Details', $AllMember->member_id)}}" class="view-loan-btn px-3 py-1 rounded bg-green-50 text-green-700 border border-green-200 hover:bg-green-200 hover:text-green-900 focus:outline-none focus:ring-2 focus:ring-green-400 transition-all shadow-sm">View Details</a>
              </td>
            </tr>
            @endforeach
          </tbody>
