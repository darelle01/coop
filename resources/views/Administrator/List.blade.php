    ...
    <header class="bg-white shadow-md fixed left-0 top-0 z-50 w-full">
      <div
        ...
        <div class="flex items-center space-x-4">
          ...
          <h1 class="text-lg md:text-xl font-semibold text-teal-900">GBLDC Loan Applications</h1>
        </div>
        <div class="relative">
      ...
      <div class="overflow-x-auto">
        <table id="loanTable" class="display responsive nowrap w-full text-sm border border-green-100 rounded-lg">
          ...
          <tbody>
            ...
            <tr>
              <td>{{$LoanApplication->last_name}}, {{$LoanApplication->first_name}} {{$LoanApplication->middle_name}}</td>
              <td>{{$LoanApplication->contact_number}}</td>
              <td>{{$LoanApplication->loan_type}}</td>
              <td>{{$LoanApplication->loan_amount}}</td>
              <td>{{$LoanApplication->created_at}}</td>
              <td>
                <a href="{{route('Review.LoanApp', $LoanApplication->id)}}" class="view-loan-btn px-3 py-1 rounded bg-green-50 text-green-700 border border-green-200 hover:bg-green-200 hover:text-green-900 focus:outline-none focus:ring-2 focus:ring-green-400 transition-all shadow-sm">View Details</a>
              </td>
            </tr>
            @endforeach
          </tbody>
