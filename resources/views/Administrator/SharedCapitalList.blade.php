...
<html lang="en">
  ...
  <body class="bg-gray-50 text-gray-800 font-sans antialiased min-h-screen">
    ...
    <header class="bg-white shadow-md fixed left-0 top-0 z-50 w-full">
      <div
        ...
        <div class="flex items-center space-x-4">
          ...
          <h1 class="text-lg md:text-xl font-semibold text-teal-900">GBLDC Shared Capital List</h1>
        </div>
        <div class="relative">
    ...
    <!-- Title -->
    <div class="mb-8">
      <h2 class="text-2xl font-bold text-green-800">Shared Capital List</h2>
      <p class="text-gray-700">View all shared capital of members.</p>
...
<th>Member ID</th>
          ...
          <tbody>
            ...
            <tr>
              <td>{{$SharedCapital->member_id}}</td>
              <td>{{$SharedCapital->last_name}}, {{$SharedCapital->first_name}} {{$SharedCapital->middle_name}}</td>
<td>{{$SharedCapital->shared_capital_amount}}</td>
              <td>{{$SharedCapital->shared_capital_amount_balance}}</td>
              <td>{{$SharedCapital->created_at}}</td>

              <td>
                @if($SharedCapital->status == 'fully paid')
                  <span class="px-2 py-1 bg-green-100 text-green-800 rounded-full text-xs font-medium">Fully Paid</span>
                @else
                  <span class="px-2 py-1 bg-blue-100 text-blue-800 rounded-full text-xs font-medium">Ongoing</span>
                @endif
              </td>
              <td>
                <a href="{{route ('View.Shared.Capital.Details', $SharedCapital->member_id)}}" class="view-loan-btn px-3 py-1 rounded bg-green-50 text-green-700 border border-green-200 hover:bg-green-200 hover:text-green-900 focus:outline-none focus:ring-2 focus:ring-green-400 transition-all shadow-sm mr-2">View Details</a>
                <a href="{{route ('View.SCP.History', $SharedCapital->member_id)}}" class="view-loan-btn px-3 py-1 rounded bg-blue-50 text-blue-700 border border-blue-200 hover:bg-blue-200 hover:text-blue-900 focus:outline-none focus:ring-2 focus:ring-blue-400 transition-all shadow-sm mr-2">History</a>
                @if($SharedCapital->status != 'fully paid')
                <form action="{{route ('Mark.Shared.Capital.Fully.Paid', $SharedCapital->member_id)}}" method="POST" style="display:inline;" onsubmit="return confirm('Are you sure you want to mark this shared capital as fully paid?');">
                  @csrf
                  <button type="submit" class="view-loan-btn px-3 py-1 rounded bg-purple-50 text-purple-700 border border-purple-200 hover:bg-purple-200 hover:text-purple-900 focus:outline-none focus:ring-2 focus:ring-purple-400 transition-all shadow-sm">Fully Paid</button>
                </form>
                @endif
              </td>
