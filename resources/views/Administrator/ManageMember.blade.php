  ...
  <html lang="en">
    <head>
      <meta charset="UTF-8" />
      <title>Manage Membership Registration | GBLDC Admin</title>
  ...
  <style>
    /* Example: Add vertical spacing between rows */
    #regisTable tbody tr {
      margin-bottom: 10px;
    }

    /* Example: Add padding to cells */
    #regisTable th {
      padding-left: 80px !important; /* override default */
    }

    /* Example: Reduce space between table controls (search, pagination) */
    ...
    <body class="bg-gray-50 text-gray-800 font-sans antialiased min-h-screen">
      ...
      <header class="bg-white shadow-md fixed left-0 top-0 z-50 w-full">
        <div
          ...
          <div class="flex items-center space-x-4">
            ...
            <h1 class="text-lg md:text-xl font-semibold text-teal-900">GBLDC Manage Membership Registration</h1>
          </div>
          <div class="relative">
      ...
      <!-- Title -->
      <div class="mb-8">
        <h2 class="text-2xl font-bold text-green-800">Manage Membership</h2>
        <p class="text-gray-700">View, approve, or reject new member registration.</p>
        <form action="{{route ('Admin.dashboard')}}" method="GET" class=" flex flex-row justify-between">
          <button class="inline-flex items-center mt-1 px-3 py-1.5 rounded bg-green-100 text-green-800 hover:bg-green-200 transition-colors focus:outline-none focus:ring-2 focus:ring-green-400 shadow-sm mr-3">
          <i class="fa fa-arrow-left mr-2"></i> Back
          </button>
          
          <a href="{{route ('Add.New.Member')}}" type="button" class="bg-green-600 hover:bg-green-700 text-white px-5 py-2 rounded-lg shadow flex items-center gap-2">
      ...
      <div class="bg-white rounded-xl shadow p-8">
        ...
        <div class="overflow-x-auto">
          <table id="regisTable" class="display responsive nowrap w-full text-sm border border-green-100 rounded-lg">
            <thead>
              <tr class="bg-green-100 text-green-900">
                <th class="py-2 px-4">Fullname</th>
                <th class="py-2 px-4 ">Email</th>
                <th class="py-2 px-4 ">Contact</th>
                <th class="py-2 px-4 ">Status</th>
                <th id="regisTable">Actions</th>
              </tr>
            </thead>
            <tbody>
              @foreach ($members as $member_detail)
              <tr>
                <td class="py-2 px-4 ">{{$member_detail->last_name}}, {{$member_detail->first_name}} {{$member_detail->middle_name}}</td>
                <td class="py-2 px-4 ">{{$member_detail->email}}</td>
                <td class="py-2 px-4 ">0{{$member_detail->contact_number}}</td>
                <td class="py-2 px-4 "><span class="bg-yellow-200 text-yellow-800 px-2 py-1 rounded">Pending</span></td>
                <td class="py-2 px-4 flex gap-2">
                    <!-- Approve Form -->
                    <form action="{{route('Approve.member')}}" method="POST">
                      @csrf
                      <input type="text" name="member_id" class=" px-4 py-2.5 text-sm text-gray-700 bg-white border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition-all duration-200" placeholder="Assign Member ID">
                      <input type="hidden" name="id" value="{{ $member_detail->id }}">
                      <button type="submit" class="bg-green-100 hover:bg-green-200 text-green-600 px-4 py-2 rounded">Approve</button>
                    </form>
                  <!-- View Form -->
                    <a href="{{route('redirect.Membershipform', $member_detail->id)}}" class="bg-blue-100 hover:bg-blue-200 text-blue-600 px-4 py-2 rounded">View</a>
    
                    <!-- Reject Form -->
                    <form action="{{route('Reject.member')}}" method="POST">
                      @csrf
                      <input type="hidden" name="id" value="{{ $member_detail->id }}">
                      <button type="submit" class="bg-red-100 hover:bg-red-200 text-red-600 px-4 py-2 rounded">Reject</button>
                    </form>
    ...
    <script>
      $(document).ready(function () {
        ...
        $('#regisTable thead tr').clone(true).appendTo('#regisTable thead');
        $('#regisTable thead tr:eq(1) th').each(function (i) {
          if (i === 2) { // Only the "Type" column
            ...
            $('#regisTable tbody tr').each(function () {
        ...
        const table = $('#regisTable').DataTable({
          ...
          language: {
            ...
            info: "Showing _START_ to _END_ of _TOTAL_ regiss",
            infoEmpty: "No regiss found",
            zeroRecords: "No matching regiss found"
          }
        });
      });
